...
import com.gmail.dengtao.joe.commons.util.SixBitUtils;
import com.gmail.dengtao.joe.commons.util.StringUtils;
import com.gmail.dengtao.joe.tools.Window;

public class SixBitPage implements TabPage {

	private Clipboard clipboard;	// 
	private TextTransfer textTransfer = TextTransfer.getInstance();
	
	private boolean SIX_BIT_PLAIN_MODIFY_FLAG = true;
	private boolean SIX_BIT_ENCODED_MODIFY_FLAG = true;
	private boolean SIX_BIT_CURRENT_MODIFY_PLAIN = true;
	private String SIX_BIT_CHARSET = CharsetUtils.UTF_8.name(); 
	...
	public void create(CTabFolder folder) {
		...
		tab.setImage(SWTResourceManager.getImage(Window.class, "/icons/icon-6bit-16.png"));
		tab.setText("6Bit");
		...
		plainText.addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent event) {
				if (SIX_BIT_PLAIN_MODIFY_FLAG) {
					SIX_BIT_CURRENT_MODIFY_PLAIN = true;
					SIX_BIT_ENCODED_MODIFY_FLAG = false;
					String plain = plainText.getText();
					encodedText.setText(SixBitUtils.encode(plain, SIX_BIT_CHARSET));
					SIX_BIT_ENCODED_MODIFY_FLAG = true;
				}
			}
		});
		...
		encodedText.addModifyListener(new ModifyListener() {
			@Override
			public void modifyText(ModifyEvent event) {
				if (SIX_BIT_ENCODED_MODIFY_FLAG) {
					SIX_BIT_CURRENT_MODIFY_PLAIN = false;
					SIX_BIT_PLAIN_MODIFY_FLAG = false;
					String encoded = encodedText.getText();
					plainText.setText(SixBitUtils.decode(encoded, SIX_BIT_CHARSET));
					SIX_BIT_PLAIN_MODIFY_FLAG = true;
				}
			}
		});
		...
		charsetRadioUTF8.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				SIX_BIT_CHARSET = CharsetUtils.UTF_8.name();
				if (SIX_BIT_CURRENT_MODIFY_PLAIN) {
					SIX_BIT_ENCODED_MODIFY_FLAG = false;
					String plain = plainText.getText();
					encodedText.setText(SixBitUtils.encode(plain, SIX_BIT_CHARSET));
					SIX_BIT_ENCODED_MODIFY_FLAG = true;
		...
		charsetRadioGBK.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				SIX_BIT_CHARSET = CharsetUtils.GBK.name();
				if (SIX_BIT_CURRENT_MODIFY_PLAIN) {
					SIX_BIT_ENCODED_MODIFY_FLAG = false;
					String plain = plainText.getText();
					encodedText.setText(SixBitUtils.encode(plain, SIX_BIT_CHARSET));
		...
		charsetRadioGB2312.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				SIX_BIT_CHARSET = CharsetUtils.GB2312.name();
				if (SIX_BIT_CURRENT_MODIFY_PLAIN) {
					SIX_BIT_ENCODED_MODIFY_FLAG = false;
					String plain = plainText.getText();
					encodedText.setText(SixBitUtils.encode(plain, SIX_BIT_CHARSET));
