...
import com.gmail.dengtao.joe.commons.util.XMLUtils;
import com.gmail.dengtao.joe.tools.Window;

public class XMLPage implements TabPage {
	...
	public void create(CTabFolder folder) {
		...
		tab.setImage(SWTResourceManager.getImage(Window.class, "/icons/icon-xml-16.png"));
		tab.setText("XML");
		...
		pretty.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				try {
					String pretty = XMLUtils.pretty(plainText.getText());
			        ...
			        messageBox.setMessage("解析XML异常：" + e.getLocalizedMessage());
			        messageBox.open();
				}
			}
		});
		...
		compact.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				try {
					String compact = XMLUtils.compact(plainText.getText());
